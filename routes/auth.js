...
router.post('/register', async (req, res) => {
  try {
    ...
    ).run(username.trim(), email.toLowerCase().trim(), hash, isAdmin);
    
    const userId = result.rows?.[0]?.id || result.lastID;
    const token = jwt.sign(
      { id: userId, username, is_admin: isAdmin === 1 },
      JWT_SECRET,
      { expiresIn: '30d' }
    );
    ...
    res.json({
      ...
      username,
      is_admin: isAdmin === 1,
      message: isAdmin ? '¡Bienvenido! Eres el administrador.' : '¡Registro exitoso!'
    });
  } catch (err) {
